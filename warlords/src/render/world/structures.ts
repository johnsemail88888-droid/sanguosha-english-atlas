...
import { PropCtx, boxMM, jitter, lantern, lattice, pillar, roofExtras, shade, mixCol, trs, PRIM, V, col } from './propkit';
import {
  BRIDGE_DECK_T,
  BRIDGE_RAIL_H,
  BRIDGE_RAIL_T,
...
export function buildTent(c: PropCtx): void {
  ...
  switch (p.variant) {
    case 1: {
      ...
      hipRoof(b, 0, wallH, 0, sx, sz, sy - wallH, { ...roofExtras(c), color: cc, overhang: 0.35, ridge: 0.5, upturn: 0.1, curve: 1.15, stripes: false, plain: true, underside: shade(cc, 0.6) });
...
export function buildShip(c: PropCtx): void {
  ...
  if (v === 2) {
    ...
    hipRoof(b, 0, 0.2, 0, sx * 0.8, sz - 1, 2.2, { ...roofExtras(c), color: '#5a4030', overhang: 0.1, ridge: 0.9, curve: 0.8, upturn: 0, stripes: false, plain: true });
  } else {
